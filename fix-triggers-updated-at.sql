-- ====================================
-- CORRECTION DES TRIGGERS updated_at / updatedAt
-- Résout le conflit entre snake_case et camelCase
-- ====================================
...
-- 2. Supprimer TOUTES les anciennes fonctions de trigger
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS update_updated_at() CASCADE;

-- 3. Créer une fonction pour les colonnes camelCase (updatedAt)
...
-- 5. Créer les triggers selon le type de colonne de chaque table

-- Tables avec camelCase (updatedAt)
...
-- Tables avec snake_case (updated_at) - seulement si elles existent
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'landing_page_content') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'legal_pages') THEN
...
-- 7. Vérification : Afficher les colonnes pour voir leur nom exact
SELECT 
  table_name,
  column_name,
  data_type
