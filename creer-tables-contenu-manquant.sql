-- ====================================
-- CRÉATION DES TABLES DE GESTION DE CONTENU
-- Pour que les pages Super Admin fonctionnent correctement
-- ====================================
...
DROP POLICY IF EXISTS "Only super_admin can modify landing page content" ON landing_page_content;
CREATE POLICY "Only super_admin can modify landing page content" ON landing_page_content
  FOR ALL USING (true); -- Temporairement désactivé pour permettre les modifications
...
DROP POLICY IF EXISTS "Only super_admin can modify footer content" ON footer_content;
...
DROP POLICY IF EXISTS "Only super_admin can modify legal pages" ON legal_pages;
...
VALUES 
  ('about', 'À propos de SolidarPay', '<h1>À propos de SolidarPay</h1><p>Contenu à compléter...</p>', 'Découvrez SolidarPay, la plateforme qui modernise les tontines', true),
...
SELECT 'Tables créées avec succès!' as status,
  (SELECT COUNT(*) FROM landing_page_content) as landing_sections,
  (SELECT COUNT(*) FROM footer_content) as footer_sections,
  (SELECT COUNT(*) FROM legal_pages) as legal_pages;
